...
public class CannonBall : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.tag == "Enemy" & collision.gameObject != _owner)
        {
            if (collision.gameObject.GetComponent<ProjetPirate.IA.Shark_Character>() != null)
            {
                _owner.Owner.GetComponent<Player>().GainXP(collision.gameObject.GetComponent<ProjetPirate.IA.Shark_Character>().Damage(_damage));
            ...
            {
                _owner.Owner.GetComponent<Player>().GainXP(collision.gameObject.GetComponent<ProjetPirate.IA.Ship_Character>().Damage(_damage));
                collision.gameObject.GetComponent<ProjetPirate.IA.Ship_Controller>().AlertFromShoot(_owner.gameObject);
            }
            Destroy(this.gameObject);
        }
        if (collision.gameObject.tag == "Player" & collision.gameObject.GetComponent<ProjetPirate.Boat.BoatCharacter>() != _owner)
        {
            if (collision.gameObject.GetComponent<ProjetPirate.Boat.BoatCharacter>() != null)
            {
                collision.gameObject.GetComponent<ProjetPirate.Boat.BoatCharacter>().Damage(_damage);
            }
            Destroy(this.gameObject);
        }
    }
