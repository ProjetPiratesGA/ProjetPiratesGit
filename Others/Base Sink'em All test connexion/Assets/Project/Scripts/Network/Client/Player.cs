...
public class Player : Controller
{
    ...
    [Command]
    public void CmdRefreshDatas()
    {
        NetworkConnection conn = this.connectionToClient;

        List<BoatController> boatList = NetworkManager.singleton.GetComponent<ServerNetworkManager>().boatList;
        List<Player> playerList = NetworkManager.singleton.GetComponent<ServerNetworkManager>().playerList;


        for (int i = 0; i < boatList.Count; i++)
        {
            if(boatList[i].player != this)
            {
                boatList[i].gameObject.GetComponent<BoatCharacter>().TargetSetParent(conn, boatList[i].player.gameObject);
            }
        }
        for (int i = 0; i < playerList.Count; i++)
        {
            if(playerList[i]._username != this._username)
            {
                //if(playerList[i].asBoatSpawned)
                //{
                    ...
                    tempPos.x = playerList[i]._data.Boat.Transform.Position.x;
                    tempPos.y = playerList[i]._data.Boat.Transform.Position.y;
                    tempPos.z = playerList[i]._data.Boat.Transform.Position.z;

                    Vector4 tmpRot = new Vector4();
                    tmpRot.y = playerList[i]._data.Boat.Transform.Rotation.y;

                    playerList[i].TargetSetStartData(conn, playerList[i]._data.Boat.Stats.Life, playerList[i]._data.Ressource.Golds, playerList[i]._data.Boat.CurrentCanonLeft
                    , playerList[i]._data.Boat.CurrentCanonRight, playerList[i]._data.Boat.MaxCanonPerSide, playerList[i]._data.Boat.Stats.Speed, tempPos, tmpRot);
                    //FIN SEB 08
               // }
            }
        }
        for (int i = 0; i < boatList.Count; i++)
        {
            if (boatList[i].player != this)
            {
                boatList[i].gameObject.GetComponent<BoatCharacter>().TargetUpdateActiveCannons(conn);
            }
        }
    }
