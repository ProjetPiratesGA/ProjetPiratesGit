    ...
    public class ServerNetworkManager : NetworkManager
    {
        ...
        public override void OnServerAddPlayer(NetworkConnection conn, short playerControllerId)
        {
            ...
            for (int i = 0; i < playerList.Count; i++)
            {
                ...
                , _playerList[i]._data.Boat.CurrentCanonRight, _playerList[i]._data.Boat.Stats.Speed, tempPos, tmpRot, _playerList[i]._data.Ressource.BoatLevel, _playerList[i]._data.Boat.AsHarpoon);
                //FIN SEB 08
            }
            for (int i = 0; i < _boatList.Count; i++)
            {     
